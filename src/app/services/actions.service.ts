...
export class ActionsService {
  ...
  private getStorage() {
    if (this.encuestas === null || this.encuestas === undefined) {
      const lSE = localStorage.getItem('encuestas');
      this.encuestas = lSE ? JSON.parse(lSE) : preguntasStorage; // Para pruebas
    }
  
    if (this.respuestas === null || this.respuestas === undefined) {
      const lSR = localStorage.getItem('respuestas');
      this.respuestas = lSR ? JSON.parse(lSR) : respuestasStorage; // Para pruebas
    }
  }
  ...
  private generateID(cadena: string, largo = 10): any{
